...
public class Polyglot {
    ...
    public static Map<String, Object> runMainFunctionWithData(
        ...
        try {
            ...
            checkReturnedMapIsNotExceptionObject(returnedMap);
    ...
    private static Context buildPolyglotCtx() {
        ...
        try {
            ctx.eval(buildJsExceptionObjCreator("reject", COMMITTABLE_EXCEPTION));
            ctx.eval(buildJsExceptionObjCreator("requeue", NOT_ACK_EXCEPTION));

            ctx.eval(buildPythonExceptionObjCreator("reject", COMMITTABLE_EXCEPTION));
            ctx.eval(buildPythonExceptionObjCreator("requeue", NOT_ACK_EXCEPTION));
        } catch (Exception e) {
            log.error("Failed to add built in error object return functions to polyglot context for!");
        }
        return ctx;
    }

    @SneakyThrows
    private static Source buildJsExceptionObjCreator(String functionName, String exceptionType) {
        val code = format("const %s = (message) => ({ %s: \"%s\", %s: message });", functionName, EXCEPTION_TYPE_KEY, exceptionType, MESSAGE_KEY);
        return Source.newBuilder("js", code, functionName).build();
    }

    @SneakyThrows
    private static Source buildPythonExceptionObjCreator(String functionName, String exceptionType) {
        val code = format("%s = lambda message : { \"%s\": \"%s\", \"%s\": message }", functionName, EXCEPTION_TYPE_KEY, exceptionType, MESSAGE_KEY);
        return Source.newBuilder("python", code, functionName).build();
    }

    private static void checkReturnedMapIsNotExceptionObject(Map<String, Object> returnedMap) throws GraphException {
        if (Objects.equals(returnedMap.get(EXCEPTION_TYPE_KEY), COMMITTABLE_EXCEPTION)) {
            throw new CommittableException(returnedMap.get(MESSAGE_KEY).toString());
        } else if (Objects.equals(returnedMap.get(EXCEPTION_TYPE_KEY), NOT_ACK_EXCEPTION)) {
            throw new NotAcknowledgeableException(returnedMap.get(MESSAGE_KEY).toString());
        }
    }
}
